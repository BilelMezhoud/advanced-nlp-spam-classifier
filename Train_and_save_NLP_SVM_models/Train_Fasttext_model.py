...
df_balanced['Message'] = df_balanced['Message'].apply(lambda x: vectorize_text_ft(x, ft_model))
X = df_balanced['Message'].tolist()
y = df_balanced['spam'].tolist()
...
grid_search = GridSearchCV(Ft_svm, parameters, cv=5)
grid_search.fit(X_train, y_train)

# Save the trained model
joblib.dump(grid_search.best_estimator_, 'Ft_svm.pkl')
...
print("Accuracy score using fasttext:", accuracy)
print("Precision score using fasttext:", precision)
print("Recall score using fasttext:", recall)
print("F1 score using fasttext:", f1)
