...
df_balanced['Message'] = df_balanced['Message'].apply(lambda x: vectorize_text_word2vec(x))
...
w2v_svm = SVC(random_state=42)

# perform hyperparameter tuning using GridSearchCV or RandomizedSearchCV
parameters = {'kernel': ['linear', 'rbf'], 'C': [1, 10, 100, 1000]}
grid_search = GridSearchCV(w2v_svm, parameters, cv=5)
grid_search.fit(X_train, y_train)

# Save the trained model
joblib.dump(grid_search.best_estimator_, 'w2v_svm_model.pkl')
...
print("Accuracy score using Word2vec:", accuracy)
print("Precision score using Word2vec:", precision)
print("Recall score using Word2vec:", recall)
print("F1 score using Word2vec:", f1)